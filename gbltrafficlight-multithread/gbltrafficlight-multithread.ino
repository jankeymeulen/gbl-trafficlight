...
/* Flag stuff */
#define FLAG_COUNT 3
#define FLAG_SEGMENTS 3
#define FLAG_DELAY 40
int flagStart = 0;
...
void fillFlag() {
  if (previousFrame + FLAG_DELAY < millis()) {

    int segmentSize = floor(NUM_LEDS/FLAG_COUNT/FLAG_SEGMENTS);

    for (int i = 0; i < NUM_LEDS; i++)
    {
      int segment = ((int)floor((i+flagStart)/segmentSize))%FLAG_SEGMENTS;
      if (segment == 0) {
        leds[i] = CRGB(xtermColours[colour.r][0],xtermColours[colour.r][1],xtermColours[colour.r][2]);
      } else if (segment == 1) {
        leds[i] = CRGB(xtermColours[colour.g][0],xtermColours[colour.g][1],xtermColours[colour.g][2]);
      } else {
        leds[i] = CRGB(xtermColours[colour.b][0],xtermColours[colour.b][1],xtermColours[colour.b][2]);
      }
    }
    flagStart++;
